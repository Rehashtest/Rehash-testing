...
TestData assetData = TestDataFactory.findTestData('Data Files/Device Reg Test Data')
...
for (int row = 1; row <= totalRows; row++) {
	...
	String CDATE = new SimpleDateFormat("yyyyMMdd").format(Calendar.getInstance().getTime());
	
	String CTIME = new SimpleDateFormat("HHmmss").format(Calendar.getInstance().getTime());
	...
	WebUI.click(findTestObject('Object Repository/Ops Device Approval (All)/Page_REHash/button_New'))
	...
	if (WebUI.verifyElementVisible(findTestObject('Object Repository/Ops Device Approval (All)/Page_REHash/td_I-REC'), FailureHandling.OPTIONAL) == true) {
		...
		WebUI.setText(findTestObject('Object Repository/Ops Admin Device Approval/Page_REHash/input__AssetCode'), CDATETIME) //(YYYY-MM-DD)find a way for asset code to be forever increasing
		...
		WebUI.click(findTestObject('Object Repository/Asset Registration IREC/Page_REHash/div_o_flex items-center'))
		...
		String CDATE2 = new SimpleDateFormat("yyyyMMdd").format(Calendar.getInstance().getTime());
		
		String CTIME2 = new SimpleDateFormat("HHmmss").format(Calendar.getInstance().getTime());
		
		String CDATETIME2 = CDATE2 + " " + CTIME2
		...
		WebUI.setText(findTestObject('Object Repository/Ops Device Approval (APX)/Page_REHash/input__AssetCode'), CDATETIME2)
